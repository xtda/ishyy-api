class TowJob < Tempjob
  ...
  def say_response(total_pay, name)
    ...
    "#{name}, a tow truck was requested to pick up #{tow_type_response} #{car_response} from #{pick_up_location_response} you got paid #{total_pay} potatoes"
  end

  def say_fail_response(total_pay, name)
